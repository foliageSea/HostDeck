...
<template>
  ...
  >
    ...
    <div
      class="flex min-w-0 items-center justify-center gap-[12px] text-[0.92rem]"
    ...
    <div class="flex min-w-0 items-center justify-end gap-[12px]">
      ...
      >
        ...
        <div
          ...
          :class="[
            settingsStore.isDark
              ? 'border border-[rgba(148,163,184,0.18)] bg-[rgba(15,23,42,0.92)] text-[#e2e8f0] shadow-[0_18px_48px_rgba(15,23,42,0.28)]'
              : 'border border-[rgba(148,163,184,0.24)] bg-[rgba(255,255,255,0.96)] text-[#0f172a] shadow-[0_18px_42px_rgba(148,163,184,0.24)]',
          ]"
        >
          <div class="flex items-start justify-between gap-[12px]">
            <div>
              <div class="text-[14px] font-700">任务中心</div>
              <div class="text-[12px]" :class="settingsStore.isDark ? 'text-[rgba(148,163,184,0.94)]' : 'text-[rgba(100,116,139,0.92)]'">
          ...
          <div v-if="uploadBatches.length === 0" class="text-[12px]" :class="settingsStore.isDark ? 'text-[rgba(148,163,184,0.94)]' : 'text-[rgba(100,116,139,0.92)]'">
            当前没有上传任务
          </div>
          ...
          <div v-else class="flex flex-col gap-[12px] overflow-auto pr-[2px]">
            ...
            >
              <div class="flex items-start justify-between gap-[12px] lt-md:flex-col">
                <div>
                  <div class="truncate-line text-[13px] font-600" :class="settingsStore.isDark ? '' : 'text-[#0f172a]'">文件上传 · {{ batch.path }}</div>
                  <div class="text-[12px]" :class="settingsStore.isDark ? 'text-[rgba(148,163,184,0.94)]' : 'text-[rgba(100,116,139,0.92)]'">
                ...
                <div class="flex flex-col items-end gap-[4px] text-[12px] lt-md:items-start" :class="settingsStore.isDark ? 'text-[rgba(191,219,254,0.96)]' : 'text-[rgba(37,99,235,0.86)]'">
              ...
              <div v-if="batch.errorMessage" class="text-[12px]" :class="settingsStore.isDark ? 'text-[rgba(248,113,113,0.96)]' : 'text-[rgba(220,38,38,0.92)]'">
                {{ batch.errorMessage }}
              </div>
              ...
              <div class="flex flex-col gap-[8px]">
                ...
                >
                  <div class="flex items-start justify-between gap-[12px] lt-md:flex-col">
                    <div class="truncate-line" :class="settingsStore.isDark ? '' : 'text-[#0f172a]'" :title="task.name">{{ task.name }}</div>
                    <NTag size="small" :type="task.status === 'error' ? 'error' : task.status === 'success' ? 'success' : 'info'">
                      {{ formatUploadStatus(task.status) }}
                    </NTag>
                  </div>
                  <div class="flex items-start justify-between gap-[12px] text-[12px] lt-md:flex-col" :class="settingsStore.isDark ? 'text-[rgba(148,163,184,0.94)]' : 'text-[rgba(100,116,139,0.92)]'">
