...
- 前端构建：`pnpm --dir host-deck-ui build`
...
- CI release 会构建 `host-deck-ui/dist`，Windows 桌面 job 下载到 `assets/web`，Dockerfile 直接复制 `host-deck-ui/dist` 到 `/app/web`。
- `bin/server.dart` 未传 `--web-dir` 时只会查找可执行文件旁 `../web`；开发运行通常要显式传 `--web-dir host-deck-ui/dist`。
...
- 前端源码入口 `host-deck-ui/src/main.ts`；API 类型/请求在 `src/api/`，Pinia 在 `src/stores/`，HTTP/UI 基础设施在 `src/lib/`，业务窗口在 `src/views/`。
