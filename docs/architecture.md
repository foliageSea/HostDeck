...
- Vue 3 前端：当前唯一前端工程位于 `host-deck-ui/`，构建产物由桌面壳或 Dart 服务作为静态资源提供。
...
- 注意：Vite 开发服务器端口以 `host-deck-ui/vite.config.ts` 的 `server.port` 为准；Electron 开发模式会读取该配置，也可通过 `HOST_DECK_ELECTRON_DEV_URL` 覆盖；`lib/main.dart` debug WebView 当前仍硬编码加载 `http://localhost:5173`。
...
- Docker 镜像直接复制 `host-deck-ui/dist` 到 `/app/web`。
...
1. 前端通过 `host-deck-ui/src/lib/http.ts` 的 Axios 实例发起请求。
