  ...
  <div class="flex flex-col md:flex-row h-full gap-6 text-gray-800 dark:text-gray-100 overflow-hidden">
    ...
    <div class="w-full md:w-1/3 lg:w-1/4 flex flex-col space-y-4 min-w-[280px]">
      ...
      <div class="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar pb-4">
        <div v-if="sshStore.savedServers.length === 0"
          class="flex flex-col items-center justify-center py-12 text-gray-400 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
          ...
          <div class="flex justify-between items-start">
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center text-xs text-gray-500 dark:text-gray-400 mt-1 space-x-2">
                <span class="bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-[10px] font-mono">{{
                  server.username }}</span>
                <span class="truncate">{{ server.host }}:{{ server.port }}</span>
              </div>
            </div>
            <button @click.stop="deleteServer(server.id)"
    ...
    <div class="flex-1 flex flex-col justify-center items-center p-4 md:p-8 overflow-y-auto">
      <div
        class="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl max-w-2xl w-full border border-gray-100 dark:border-gray-700 transition-all">
        ...
        <form @submit.prevent="connect" class="space-y-5">
          ...
          <div class="relative">
            ...
            <input v-model="form.name" type="text" placeholder="例如：生产环境数据库"
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div class="md:col-span-2">
              ...
              <input v-model="form.host" type="text" placeholder="192.168.1.1"
          ...
          <div>
            ...
            <textarea v-model="form.privateKey" placeholder="-----BEGIN RSA PRIVATE KEY-----"
              class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all font-mono text-xs h-24 resize-none"></textarea>
          </div>
          ...
          <div class="pt-4 flex gap-4">
            ...
            <button type="button" @click="saveServer"
...
function selectServer(server: SavedServer) {
  currentServerId.value = server.id ?? null
...
function deleteServer(id: number | undefined) {
