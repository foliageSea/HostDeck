...
export function getComposeStatusType(project: DockerComposeProject) {
  ...
  if (status.includes('exited') || status.includes('stopped')) return 'warning'
  return 'default'
}
...
export function getComposeServiceStatusType(service: DockerComposeService) {
  ...
  if (state.includes('running')) return 'success'
  if (state.includes('exit') || state.includes('stop')) return 'warning'
  return 'default'
}
