...
import { dockerApi, type DockerComposeProject, type DockerComposeProjectPayload, type DockerComposeService } from '@/api/docker'
import { getUiApi } from '@/lib/ui'
import { useDesktopStore } from '@/stores/desktop'
import { useSettingsStore } from '@/stores/settings'
import { useSshStore } from '@/stores/ssh'
import { getComposeConfigFiles, getComposeProjectPayload, getComposeServiceStatusType } from '../hooks/dockerViewHelpers'
...
<template>
  <div
    ...
    :class="settingsStore.isDark ? 'docker-theme-dark bg-[linear-gradient(180deg,rgba(15,23,42,0.18),rgba(15,23,42,0.06))]' : 'docker-theme-light bg-[linear-gradient(180deg,rgba(255,255,255,0.7),rgba(226,232,240,0.36))]'"
  >
    <div
    ...
    >
      <div class="min-w-0">
        ...
        <div class="flex flex-wrap gap-[8px] text-[12px]" :class="settingsStore.isDark ? 'text-[rgba(226,232,240,0.58)]' : 'text-[rgba(71,85,105,0.78)]'">
    ...
    <NSpin :show="loading" class="compose-services-body app-scrollbar" :class="settingsStore.isDark ? 'app-scrollbar-dark' : 'app-scrollbar-light'">
      <NEmpty v-if="services.length === 0 && !loading" description="未加载到编排服务" />

      <div v-else class="compose-service-grid">
        <NCard v-for="service in services" :key="service.id || service.name" size="small" :bordered="false" class="compose-service-card">
          <template #header>
            <div class="min-w-0">
          ...
          <template #header-extra>
            <NTag round size="small" :type="getComposeServiceStatusType(service)">{{ service.state || service.status || 'unknown' }}</NTag>
          </template>
